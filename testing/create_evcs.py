...
def execute(circuit_n: int):
    tags_a = []
    tags_b = []
    def create_evc(tag_a, tag_b):
        b = "curl -H 'Content-type: application/json' -X POST http://localhost:8181/api/kytos/mef_eline/v2/evc/ -d "
        a = '{"name": "my evc1", "dynamic_backup_path": true, "enabled": true, "uni_a": {"tag": {"value": '
        a += str(tag_a)
    ...
    for i in range(1, circuit_n*2, 2):
        ...
        tags_a.append(tags)
        tags_b.append(tags)
        t = threading.Thread(target=create_evc, args=(tags, tags))
        threads.append(t)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("No number detected, 3 circuits will be created.")
    execute(circuit_n)